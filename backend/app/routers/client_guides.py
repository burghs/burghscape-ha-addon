...
async def guides_page(request: Request, db: AsyncSession = Depends(get_db)):
    user = await portal_user(request, db); guides = await visible_guides(db, user.client_id)
    cards = "".join(f'''<article class="guide-card" id="{g.id}" data-guide-id="{g.id}" data-updated="{escape(g.updated_at.isoformat() if g.updated_at else '')}">
      <div class="preview">{f'<img src="/api/portal/guides/{g.id}/file" alt="Preview of {escape(g.title)}">' if g.mime_type.startswith('image/') else '<div class="pdf-mark">PDF</div>'}</div>
      ...
      <button onclick="openGuide({g.id},'{g.mime_type}')">Open Guide</button></div></article>''' for g in guides)
...
body{{background:#030712;color:#e5e7eb;font-family:Inter,system-ui;margin:0}}nav{{padding:16px;border-bottom:1px solid #ffffff1a;background:#111827dd}}nav a{{color:#c4b5fd;text-decoration:none;font-weight:700}}main{{max-width:1200px;margin:auto;padding:28px 18px}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:20px}}.guide-card,.empty{{background:#111827;border:1px solid #ffffff1a;border-radius:18px;overflow:hidden}}.preview{{height:220px;background:#0b1020;display:flex;align-items:center;justify-content:center;overflow:hidden}}.preview img{{width:100%;height:100%;object-fit:cover;object-position:top}}.pdf-mark{{font-size:34px;font-weight:800;color:#fca5a5}}.content{{padding:20px}}.content h2{{font-size:20px;margin:10px 0}}.content p{{color:#9ca3af;min-height:48px}}.meta{{display:flex;justify-content:space-between;color:#c4b5fd;font-size:12px;text-transform:uppercase;letter-spacing:.08em}}button,.download{{display:inline-flex;margin-top:16px;background:#7c3aed;color:white;border:0;border-radius:12px;padding:11px 15px;font-weight:700;text-decoration:none;cursor:pointer}}.new-badge{{background:#2563eb;color:white;padding:2px 7px;border-radius:999px}}.new-badge.seen{{display:none}}.viewer{{position:fixed;inset:0;background:#030712ed;z-index:90;padding:18px;display:flex;flex-direction:column}}.viewer.hidden{{display:none}}.viewer-head{{display:flex;justify-content:flex-end;gap:10px}}.viewer-body{{overflow:auto;text-align:center;flex:1;margin-top:12px}}.viewer img{{max-width:none;width:auto;min-width:min(100%,900px);height:auto}}.viewer iframe{{width:100%;height:100%;border:0;background:white}}.empty{{padding:40px;text-align:center;grid-column:1/-1}}@media(max-width:640px){{.viewer{{padding:8px}}.preview{{height:180px}}}}
...
const seenKey='mybeacon-guide-seen-v1';let seen=JSON.parse(localStorage.getItem(seenKey)||'{{}}');document.querySelectorAll('.guide-card').forEach(c=>{{if(seen[c.dataset.guideId]===c.dataset.updated)c.querySelector('.new-badge').classList.add('seen')}});function openGuide(id,mime){{const card=document.getElementById(String(id));seen[id]=card.dataset.updated;localStorage.setItem(seenKey,JSON.stringify(seen));card.querySelector('.new-badge').classList.add('seen');const url='/api/portal/guides/'+id+'/file';document.getElementById('download').href=url+'?download=true';document.getElementById('viewer-body').innerHTML=mime==='application/pdf'?'<iframe title="Guide PDF" src="'+url+'"></iframe>':'<img alt="Guide" src="'+url+'">';document.getElementById('viewer').classList.remove('hidden')}}function closeGuide(){{document.getElementById('viewer').classList.add('hidden');document.getElementById('viewer-body').textContent=''}}if(location.hash){{const c=document.getElementById(location.hash.slice(1));if(c)c.querySelector('button').click()}}
</script></body></html>''')
