...
def tour_enabled(): return get_settings().CLIENT_ONBOARDING_TOUR_ENABLED
def require_tour_enabled():
    if not tour_enabled(): raise HTTPException(409,"Client onboarding tour is disabled")
...
def payload(s):
    ...
    if s is None: return {"enabled":enabled,"disabled_reason":None if enabled else "Client onboarding tour is disabled","onboarding_version":CURRENT_ONBOARDING_VERSION,"status":"not_started","current_step":0,"started_at":None,"completed_at":None,"skipped_at":None,"last_replay_at":None,"replay_active":False,"should_start":enabled}
    return {"enabled":enabled,"disabled_reason":None if enabled else "Client onboarding tour is disabled","onboarding_version":s.onboarding_version,"status":s.status,"current_step":s.current_step,"started_at":_iso(s.started_at),"completed_at":_iso(s.completed_at),"skipped_at":_iso(s.skipped_at),"last_replay_at":_iso(s.last_replay_at),"replay_active":s.replay_active,"should_start":enabled and (s.status in {"not_started","in_progress"} or s.replay_active)}
...
async def start_onboarding(request:Request,db:AsyncSession=Depends(get_db)):
    user=await portal_user(request,db);require_tour_enabled();state=await ensure_state(db,user.id)
...
async def save_step(body:StepUpdate,request:Request,db:AsyncSession=Depends(get_db)):
    user=await portal_user(request,db);require_tour_enabled();state=await current_state(db,user.id)
...
async def skip_onboarding(request:Request,db:AsyncSession=Depends(get_db)):
    user=await portal_user(request,db);require_tour_enabled();state=await ensure_state(db,user.id);now=datetime.utcnow()
