...
CTA_TYPES = {"none", "details", "portal", "support", "interest", "whatsapp", "email", "phone", "external", "custom"}
...
def client_action_url(campaign: Campaign) -> Optional[str]:
    ...
    if campaign.call_to_action_type in {"support", "interest"}:
        return f"/portal/whats-new?campaign_id={campaign.id}&interest=1"
...
def client_payload(campaign: Campaign, is_read: bool) -> dict:
    return {
        ...
        "body_content": campaign.body_content, "price_text": campaign.price_text,
        "regular_price_text": campaign.regular_price_text,
        "call_to_action_type": "interest" if campaign.call_to_action_type in {"support", "interest"} else campaign.call_to_action_type,
        "call_to_action_label": "I'm Interested" if campaign.call_to_action_type in {"support", "interest"} else campaign.call_to_action_label,
