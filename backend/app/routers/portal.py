...
<head>
    ...
    <style>
        * {{ box-sizing: border-box; }}
        ...
        .onboarding-dimmer {{ position:fixed; z-index:60; background:rgba(3,7,18,.78); pointer-events:none; }}
        .onboarding-spotlight {{ position:fixed; z-index:61; border:3px solid #a78bfa; border-radius:14px; box-shadow:0 0 24px rgba(167,139,250,.52); background:transparent; pointer-events:none; transition:top .18s ease,left .18s ease,width .18s ease,height .18s ease; }}
...
<body class="bg-gray-950 text-gray-200 min-h-screen bg-grid" id="app-body">
    ...
    <nav class="card border-b border-white/10 px-4 md:px-6 py-3">
        <div class="max-w-7xl mx-auto flex items-center justify-between gap-3">
            ...
            <div class="hidden sm:flex shrink-0 items-center gap-4 text-sm">
                <span class="text-gray-400">{user_name}</span><a id="guides-nav-desktop" data-onboarding-target="guides" href="/portal/guides" class="nav-link rounded-lg border border-blue-400/25 bg-blue-500/10 px-3 py-1.5 font-semibold text-blue-200">Guides &amp; Help <span id="guides-new-desktop" class="hidden ml-1 rounded-full bg-blue-500 px-2 py-0.5 text-xs text-white">New</span></a><a id="campaign-nav-desktop" data-onboarding-target="campaigns" href="/portal/whats-new" class="nav-link text-gray-400 hover:text-purple-300">What’s New <span id="campaign-unread-desktop" class="hidden badge badge-primary ml-1"></span></a><a data-onboarding-target="getting-started" href="/portal/getting-started" class="{setup_nav_class}">{setup_nav_label}</a>
        ...
        <div id="mobile-nav" class="hidden sm:hidden max-w-7xl mx-auto mt-3 grid gap-2 border-t border-white/10 pt-3 text-sm">
            <span class="text-gray-400 px-2">{user_name}</span><a id="guides-nav-mobile" href="/portal/guides" class="touch-action justify-start border border-blue-400/20 bg-blue-500/10 text-blue-200">Guides &amp; Help <span id="guides-new-mobile" class="hidden ml-1 rounded-full bg-blue-500 px-2 py-0.5 text-xs text-white">New</span></a><a id="campaign-nav-mobile" href="/portal/whats-new" class="touch-action justify-start text-gray-300">What’s New <span id="campaign-unread-mobile" class="hidden badge badge-primary ml-1"></span></a><a href="/portal/getting-started" class="touch-action justify-start text-purple-300">{setup_nav_label}</a>
    ...
    <main class="max-w-7xl mx-auto px-4 py-5 sm:px-6 sm:py-7">
        ...
        <section id="featured-guide-panel" class="hidden mb-4 rounded-2xl border border-blue-400/25 bg-blue-500/10 p-5 sm:mb-6"><p class="text-xs font-semibold uppercase tracking-wider text-blue-200">Guides &amp; Help</p><h2 id="featured-guide-title" class="mt-2 text-xl font-bold text-white"></h2><p id="featured-guide-description" class="mt-2 text-sm text-gray-300"></p><a id="featured-guide-link" href="/portal/guides" class="compact-action mt-4">Open Guide</a></section>
        <div id="guides-feature-spotlight" class="portal-modal hidden" role="dialog" aria-modal="true" aria-labelledby="guides-spotlight-title"><div class="card modal-panel max-w-lg p-6" tabindex="-1"><p class="text-xs font-semibold uppercase tracking-wider text-blue-300">New feature</p><h2 id="guides-spotlight-title" class="mt-2 text-2xl font-bold text-white">Guides &amp; Help</h2><p class="mt-3 text-gray-300">Find step-by-step instructions for connecting the Home Assistant app, using your MyBeacon URL and resolving common setup issues.</p><div class="mt-6 flex flex-wrap justify-end gap-2"><button id="dismiss-guides-spotlight" class="touch-action border border-white/10 text-gray-200">Not now</button><a href="/portal/guides" class="compact-action">View Guides</a></div></div></div>
    ...
    <script>
        const guideSeenKey="mybeacon-guide-seen-v1";
        const guideSpotlightKey="mybeacon-guides-spotlight-v1";
        fetch("/api/portal/guides",{{credentials:"include"}}).then(r=>r.ok?r.json():Promise.reject()).then(guides=>{{const seen=JSON.parse(localStorage.getItem(guideSeenKey)||"{{}}");const unseen=guides.some(g=>seen[g.id]!==g.updated_at);document.getElementById("guides-new-desktop").classList.toggle("hidden",!unseen);document.getElementById("guides-new-mobile").classList.toggle("hidden",!unseen);const featured=guides.find(g=>g.featured);if(featured){{document.getElementById("featured-guide-title").textContent=featured.title;document.getElementById("featured-guide-description").textContent=featured.description;document.getElementById("featured-guide-link").href="/portal/guides#"+featured.id;document.getElementById("featured-guide-panel").classList.remove("hidden")}}if(!localStorage.getItem(guideSpotlightKey)){{document.getElementById("guides-feature-spotlight").classList.remove("hidden")}}}}).catch(()=>{{}});
        document.getElementById("dismiss-guides-spotlight").addEventListener("click",()=>{{localStorage.setItem(guideSpotlightKey,"dismissed");document.getElementById("guides-feature-spotlight").classList.add("hidden")}});
