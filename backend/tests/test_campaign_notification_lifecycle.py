...
class CampaignNotificationLifecycleTests(unittest.TestCase):
 ...
 def test_security_and_client_contracts(self):
  ...
  payload=campaigns.client_payload(support,False);self.assertEqual(payload["call_to_action_type"],"interest");self.assertEqual(payload["call_to_action_label"],"I'm Interested");self.assertEqual(payload["call_to_action_url"],"/portal/whats-new?campaign_id=12&interest=1");self.assertEqual(campaign_popups.popup_payload(support)["call_to_action_url"],payload["call_to_action_url"]);support.call_to_action_url=None;self.assertEqual(campaign_popups.popup_payload(support)["call_to_action_url"],"/portal/whats-new?campaign_id=12&interest=1")
