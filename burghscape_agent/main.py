...
async def setup_tunnel(platform: PlatformClient) -> bool:
    ...
    if not hostname:
        hostname = config.instance_name.lower().replace(" ", "-") + ".mybeacon.co.za"
        logger.warning(f"Platform provided no hostname, using fallback: {hostname}")
...
async def main_loop():
    ...
    logger.info("Burghscape Agent starting (v0.2.38)")
